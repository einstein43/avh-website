...
export default function WhyChooseUsSection({ translations }: WhyChooseUsSectionProps) {
  ...
  return (
    <section ref={elementRef} className="py-20 px-4 sm:px-6 lg:px-8 bg-slate-900/50 relative overflow-hidden">
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-indigo/5 rounded-full blur-3xl" />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {whyUs.reasons.map((reason, idx) => {
            ...
            return (
              ...
              >
                <div className="bg-gradient-to-br from-indigo/30 to-sunflower/30 backdrop-blur p-6 rounded-xl border border-sunflower/20 hover:border-sunflower/40 transition-all duration-300 h-full group">
                  <div className="w-12 h-12 bg-sunflower/20 rounded-lg flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                    <Icon className="h-6 w-6 text-sunflower" />
                  </div>
                  
                  <h3 className="text-xl font-bold text-linen mb-3">
                    {reason.title}
                  </h3>
